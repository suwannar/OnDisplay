...
// Small object for holding important configuration data.
 		Instagram.Config = {
 		clientID: '164e781f151c4bb0862cacbf48bc3b86',
...
(function(){
  ...
  function toScreen(photos){
    paginated_ctr = paginated_ctr + 1;
    if (paginated_ctr <= 1500) {
	    var photos_html = '';
  ...
  function generateResource(tag){
    ...
    if(typeof tag === 'undefined'){
      throw new Error("Resource requires a tag. Try searching for selfie.");
    } else {
 ...
 function title(){
  ...
  function showPhoto(p){
    $(p).fadeIn({queue: false, duration: 'fast'});
...
$(function(){
  ...
  // Start with a search on selfie.
  Instagram.App.search('selfie');  
});
